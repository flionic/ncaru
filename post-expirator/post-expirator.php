...
function expirationdate_meta_box($post) {
	...
	$rv[] = '<label for="enable-expirationdate">'.__('Активировать','post-expirator').'</label></p>';

	if ($default == 'publish') {
		$rv[] = '<em>'.__('The published date/time will be used as the expiration value','post-expirator').'</em><br/>';
	} else {
		$rv[] = '<table><tr>';
		$rv[] = '<th style="text-align: left;">'.__('Год','post-expirator').'</th>';
		$rv[] = '<th style="text-align: left;">'.__('Месяц','post-expirator').'</th>';
		$rv[] = '<th style="text-align: left;">'.__('День','post-expirator').'</th>';
		$rv[] = '</tr><tr>';
		$rv[] = '<td>';
		...
		$rv[] = '<th style="text-align: left;">'.__('Час.','post-expirator').'</th>';
		$rv[] = '<th style="text-align: left;">'.__('Мин.','post-expirator').'</th>';
		$rv[] = '</tr><tr>';
		$rv[] = '<td>@</td><td>';
	...
	echo '<br/>'.__('Действие по окончанию','post-expirator').':<br/> ';
	...
	?>
		...
		foreach ($types as $type) {
			...
			<table class="form-table">
				...
				<tr valign-"top">
					<th scope="row"><label for="expirationdate_expiretype-<?php echo $type ?>"><?php _e('Действие по окончанию:','post-expirator'); ?></label></th>
					<td>
...
function _postExpiratorExpireType($opts) {
	...
	$rv[] = '<option value="draft" '. ($selected == 'draft' ? 'selected="selected"' : '') . '>'.__('В черновик','post-expirator').'</option>';
	$rv[] = '<option value="delete" '. ($selected == 'delete' ? 'selected="selected"' : '') . '>'.__('Удалить','post-expirator').'</option>';
	$rv[] = '<option value="private" '. ($selected == 'private' ? 'selected="selected"' : '') . '>'.__('Сделать личным','post-expirator').'</option>';
	if ($type != 'page') {
		$rv[] = '<option value="category" '. ($selected == 'category' ? 'selected="selected"' : '') . '>'.__('Заменить категорию','post-expirator').'</option>';
		$rv[] = '<option value="category-add" '. ($selected == 'category-add' ? 'selected="selected"' : '') . '>'.__('Добавить в категорию','post-expirator').'</option>';
		$rv[] = '<option value="category-remove" '. ($selected == 'category-remove' ? 'selected="selected"' : '') . '>'.__('Удалить из категории','post-expirator').'</option>';
	}
	$rv[] = '</select>';
	return implode("<br/>\n",$rv);
}
