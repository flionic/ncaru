...
function cptbc_image_url(){
	...
	?>
	<label><?php _e('Ссылка слайда', 'cpt-bootstrap-carousel'); ?>:</label>
	<input name="cptbc_image_url" value="<?php echo $cptbc_image_url; ?>" /> <br />
	<small><em><?php _e('(Оставить пустым, если ссылка не нужна)', 'cpt-bootstrap-carousel'); ?></em></small><br /><br />
	
	<label><input type="checkbox" name="cptbc_image_url_openblank" <?php if($cptbc_image_url_openblank == 1){ echo ' checked="checked"'; } ?> value="1" /> <?php _e('Ссылка в новой вкладке', 'cpt-bootstrap-carousel'); ?></label><br /><br />
	
	<label><?php _e('Текст на кнопке', 'cpt-bootstrap-carousel'); ?>:</label>
	<input name="cptbc_image_link_text" value="<?php echo $cptbc_image_link_text; ?>" /> <br />
	<small><em><?php _e('(Отображается только если включены кнопки с ссылкой)', 'cpt-bootstrap-carousel'); ?></em></small>
	<?php
}
